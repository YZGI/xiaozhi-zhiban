...
/* 全局指针，供本地 ASR 关键词拦截调用播放/停止 */
static mcp_handler_t *g_mcp = NULL;
static int g_tv_playing = 0;

/* 判断用户文本是否为电视指令：1=播放 2=停止 0=无关 */
int tv_cmd_type(const char *text)
{
    if (!text) return 0;
    static const char *stop_keys[] = {"关电视", "关闭电视", "停止播放", "退出电视",
        "不看了", "关掉电视", "别看了", "停电视", "退出视频", "关视频", "关了电视"};
    static const char *play_keys[] = {"看电视", "电视", "频道", "看视频", "播节目",
        "播放视频", "watch", "tv", "电影", "看个电视", "打开电视", "电视频道"};
    int i;
    for (i = 0; i < (int)(sizeof(stop_keys)/sizeof(stop_keys[0])); i++)
        if (strstr(text, stop_keys[i])) return 2;
    for (i = 0; i < (int)(sizeof(play_keys)/sizeof(play_keys[0])); i++)
        if (strstr(text, play_keys[i])) return 1;
    return 0;
}
...
int mcp_play_tv(mcp_handler_t *mcp, const char *url)
{
    ...
    {
        PLOG_E("TV", "播放库未加载 (splayer_set_file/play 为空)");
        return -1;
    }
    if (mcp->splayer_stop) mcp->splayer_stop();
    if (mcp->splayer_open) mcp->splayer_open();
    mcp->splayer_set_file(url ? url : TV_DEFAULT_URL);
    if (mcp->splayer_set_volume) mcp->splayer_set_volume(TV_DEFAULT_VOL);
    mcp->splayer_play();
    g_tv_playing = 1;
    PLOG_I("TV", "开始播放电视: %s", url ? url : TV_DEFAULT_URL);
    return 0;
}
...
int tv_is_playing(void) { return g_tv_playing; }
...
static int exec_tool(mcp_handler_t *mcp, const char *name, const char *args_json, size_t args_len, char *result, int result_size)
{
    ...
    if (strcmp(name, "self.play_tv") == 0)
    {
        ...
        snprintf(result, result_size, "%s", r == 0 ? "playing" : "player lib not loaded");
        return r;
    }
